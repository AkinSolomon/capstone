...
void eigenFaceTrainer(){
	...
	model->save("C:/FDB/yaml/eigenface.yml");
...
void fisherFaceTrainer(){
	...
	try{		
		string filename = "C:/at.txt";
		cout << filename << endl;
		dbread(filename, images, labels);	//this reads all the images from the file
	...
	model->save("C:/Users/Yonglan/Documents/capstone/fisherface.yml");
...
int  FaceRecognition(){
	...
	Ptr<FaceRecognizer>  model = createFisherFaceRecognizer();
	model->load("C:/Users/Yonglan/Documents/capstone/fisherface.yml");

	/*use a sample to retrieve image width and height*/
	Mat testSample = imread("C:/Users/Yonglan/Documents/capstone/at/s9/5.pgm", 0);
	...
	if (!cap.isOpened())
	{
		cout << "exit: cannot open the camera" << endl;
		return -1;
	}
